...
OAuth 토큰 검증 + JWT 발급. 참조: `server/auth/auth-router.js:27-61` (라우터 등록 `index.js:39`).

**공통 Request body** (`auth-router.js:28-31`):
```json
{
  "provider": "string (required, 'google' | 'kakao' | 'guest')",
  "token": "string (required) — provider별 의미 상이 (아래 참조)",
  "name": "string (optional, guest 한정, 20자까지 절단)"
}
```

**Provider별 `token` 의미**:

| provider | `token` 필드 | 검증 로직 |
|----------|-------------|----------|
| `google` | Google OAuth ID Token (JWT) | `OAuth2Client.verifyIdToken({ idToken, audience: GOOGLE_CLIENT_ID })` (`auth-router.js:8-15`). payload 에서 `sub` / `name` / `picture` 추출. |
| `kakao` | Kakao Access Token (Bearer) | `GET https://kapi.kakao.com/v2/user/me` 호출. `id` / `kakao_account.profile.{nickname, profile_image_url}` 추출 (`auth-router.js:17-25`). |
| `guest` | 임의 문자열 (검증 없음) | `providerId = "guest_{timestamp}_{6자랜덤}"`, `name = (req.body.name || 'Guest').slice(0, 20)`, `avatarUrl = null` (`auth-router.js:36-39`). |

**Response 200** (`auth-router.js:55-56`):
```json
{
  "jwt": "string (required, JWT signed with JWT_SECRET, payload={userId, name, provider})",
  "user": {
    "id": "number (required, DB upsertUser 반환 id)",
...
| 상태 | 조건 | 본문 | 근거 |
|------|------|------|------|
| 400 | `provider` 또는 `token` 누락 | `{ "error": "provider and token required" }` | `auth-router.js:29-31` |
| 400 | 미지원 provider | `{ "error": "unsupported provider" }` | `auth-router.js:44-45` |
| 401 | OAuth 검증 실패·`GOOGLE_CLIENT_ID` 미설정·Kakao 토큰 무효·기타 예외 | `{ "error": "token verification failed" }` | `auth-router.js:57-60` |

> DB 측면효과: 모든 성공 경로에서 `db.upsertUser({provider, providerId, name, avatarUrl})` 호출 (`auth-router.js:48-53`). guest 포함 — 매 로그인마다 새로운 `providerId` 이므로 신규 row 생성됨.
...
### S→C payload 스키마 (로비 + 상태 브로드캐스트 10종)
...
```json
{
  "rooms": "RoomInfo[] (required, 서버 보유 방 전체. 빈 배열 가능)"
}
```
**발송 시점**: 로비 WS 연결 직후 개인 전송 (`server/index.js:217-218`).
**근거**: `server/index.js:217-218`, `server/game/room.js:66-77` (RoomInfo), `lib/providers/online_game_provider.dart:310-317`
...
```json
{
  "room": "RoomInfo (required, 신규 등록된 방)"
}
```
**발송 시점**: `registerRoom` 호출 직후 로비 전체 브로드캐스트 (`POST /api/rooms`, `POST /api/quickmatch` 경유).
**근거**: `server/index.js:88`, `lib/providers/online_game_provider.dart:318-322`
...
```json
{
  "room": "RoomInfo (required, 갱신된 최신 상태)"
}
```
**발송 시점**: `broadcastRoomUpdate(room)` — 참가·이탈·호스트 변경·게임 시작·게임 종료 등 RoomInfo 에 반영되는 모든 이벤트.
**근거**: `server/index.js:257-259`, `lib/providers/online_game_provider.dart:323-332`
...
```json
{
  "roomId": "string (required, UUID v4) — 삭제된 방 id"
}
```
**발송 시점**: Room `empty` 이벤트 (전원 제거) 또는 `DELETE /api/rooms/:roomId`.
**근거**: `server/index.js:67, 162`, `lib/providers/online_game_provider.dart:333-338`
...
```json
{
  "playerCount": "number (required, 참가 후 방 인원)",
  "players": "string[] (required, 방 내 플레이어 이름 배열, addPlayer 반환 기준)"
}
```
**범위**: 방 내 본인 제외 전체 브로드캐스트 (`excludeId = result.playerId`).
**근거**: `server/index.js:525-528`, `server/game/room.js:128-135`, `lib/providers/online_game_provider.dart:363-368`
...
```json
{
  "playerId": "string (required, UUID v4) — 끊긴 플레이어 id"
}
```
**범위**: 방 내 본인 제외 브로드캐스트. WS `close` 또는 `error` 이벤트에서 `handleWsDisconnect` 경유.
**근거**: `server/index.js:319-320`, `lib/providers/online_game_provider.dart:581-588`
...
```json
{
  "playerId": "string (required, UUID v4) — 재접속한 플레이어 id"
}
```
**범위**: 방 내 본인 제외 브로드캐스트. `handleReconnect` 성공 시.
**근거**: `server/index.js:561`, `lib/providers/online_game_provider.dart:570-579`
...
```json
{
  "reason": "string (required, 한글 사유 — 예: '플레이어가 방을 떠났습니다.', '연결 시간 초과로 퇴장되었습니다.')",
  "players": "string[] (required, 제거 후 남은 플레이어 이름 배열)"
}
```
**범위**: 방 전체 브로드캐스트 (본인 포함). `handleLeaveGame` 또는 Room `disconnectTimeout` 이벤트.
**근거**: `server/index.js:72-75, 753-756`, `lib/providers/online_game_provider.dart:661-665`
...
```json
{
  "playCount": "number (required, 현재까지 play 선택 수. 최초 발송 시 0)",
  "foldCount": "number (required, 현재까지 fold 선택 수. 최초 발송 시 0)",
  "remaining": "number (required, 미선택 플레이어 수)",
  "currentPlayerId": "string (required, 다음 선택 차례 플레이어 id. 모두 결정된 경우 마지막 선택자 id)",
  "lastChoice": {
    "playerId": "string (optional, 직전 선택자 id — 최초 발송 시 없음)",
    "choice": "string (optional, 'play' | 'fold')"
  }
}
```
**범위**: 방 전체 브로드캐스트. startGame 직후(최초: `playCount=0, foldCount=0, lastChoice=없음`) 및 매 `playOrFoldResponse` 후.
**근거**: `server/index.js:590-594, 818-824`, `server/game/room.js:361-368`, `lib/providers/online_game_provider.dart:604-621`
...
```json
{
  "dealerCards": "object ({ <playerId>: Card }) — 각 플레이어가 뽑은 딜러 결정용 카드 1장",
  "dealerId": "string (required, UUID v4) — 딜러 버튼 보유자 (가장 높은 카드)",
  "playerOrder": "string[] (required) — 딜러 기준으로 재정렬된 플레이어 id 배열 (딜러가 배열 0번)"
}
```
**범위**: 방 전체 브로드캐스트. `startGame` 직후 항상 발송 (2~4인: 이후 바로 gameStart / 5~6인: 이후 POF 단계).
**근거**: `server/index.js:574-578`, `server/game/room.js:261-290`, `lib/providers/online_game_provider.dart:590-603`
...
```json
{
  "playerId": "string (required, UUID v4) — 이모트를 보낸 플레이어 id (서버가 주입)",
  "emote_id": "string|number (required) — 클라이언트가 보낸 emote_id 값 그대로 전달 (서버 검증 없음)"
}
```
**범위**: 방 전체 브로드캐스트 (본인 포함).
**근거**: `server/index.js:837`

> **브로드캐스트 미존재 메시지**: `cardPlaced`, `cardUnplaced`, `cardDiscarded`, `unDiscardCard` 는 **S→C 타입으로 존재하지 않는다**. 클라이언트의 카드 이동 요청은 서버가 개별 `stateUpdate` (`broadcastIndividual` 경유, `room.js:1026-1033`) 로 반영하며, 라인 완성 시에만 추가로 `lineCompleted` 가 별도 발송된다 (`server/index.js:641`, `server/game/room.js:532-536`).
...
| 2026-04-17 | v0.3 | /auth/verify 3-provider 스키마(Google/Kakao/Guest) + S→C 11개 payload(roomList/roomCreated/roomUpdated/roomDeleted/playerJoined/playerDisconnected/playerReconnected/playerLeft/playOrFoldUpdate/dealerSelection/emote) 실측 값으로 추가. cardPlaced 등 4개는 S→C 타입 부재 명시 (stateUpdate + lineCompleted 로 반영). | TECH | RW-5 Task #16: 나머지 TODO payload 스키마 확정 |
