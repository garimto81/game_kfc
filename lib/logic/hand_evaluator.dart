...
/// 트립스+ 또는 Top QQ+ FL 완성 가능 여부 판정.
bool _isExcitingResult(HandResult r, bool isTop) {
  if (r.handType.value >= HandType.threeOfAKind.value) return true;
  if (isTop && r.handType == HandType.onePair && r.kickers.isNotEmpty) {
    return r.kickers[0] >= 12; // Queen=12, King=13, Ace=14
  }
  return false;
}
...
Set<Card> findExcitingCards(
    List<Card> hand, List<Card> top, List<Card> mid, List<Card> bottom) {
  final exciting = <Card>{};
  final lines = [
    (top, 3, 'top'),
    (mid, 5, 'mid'),
  ...
  for (final card in hand) {
    for (final (lineCards, maxCards, lineName) in lines) {
      if (lineCards.length >= maxCards) continue;
      if (isImpactPlacement(card, lineName, lineCards, maxCards)) {
        exciting.add(card);
        break; // 이 카드는 이미 excited, 다음 카드로
      }
    }
  }
  return exciting;
...
/// 카드를 라인에 배치했을 때 트립스+/QQ+ FL이 완성되는지 판정 (배치 임팩트용).
